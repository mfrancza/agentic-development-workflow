...
Optional: `AGENT_MODEL` (default `sonnet`), `AGENT_MAX_TURNS` (default `100`), `ESCALATION_ASSIGNEE` (GitHub username to assign when `resolve-conflicts` applies the `human-required` label; omit to label without assigning), `ADMIN_ASSIGNEES` (JSON-encoded list of GitHub usernames sourced from the `ADMIN_ASSIGNEES` Actions variable; the grooming agent assigns every username in this list when it applies the `human-required` label to an issue — omit or set to `[]` to skip assignment and log a warning), `CODE_REVIEWERS` (JSON-encoded list of GitHub usernames sourced from the `CODE_REVIEWERS` Actions variable; the developer agent requests every username in this list as a PR reviewer when the change warrants human review — omit or set to `[]` to skip the request and log a warning).

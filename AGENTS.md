...
- **Bash safety in inline scripts.** Inline `run:` scripts start with `set -euo pipefail`, validate required env vars with `${VAR:?message}`, and follow the **Shell Script Conventions** above. Multi-step scripts long enough to warrant it should be extracted into `docker/scripts/` or `.github/actions/` rather than inlined.

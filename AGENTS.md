...
2. For issues labeled `plan` by the grooming agent, applying the `agent:design` label is intended to run the designer agent (`AGENT_ACTION=design`), which creates the `design/issue-{N}` branch, writes a design document in `docs/design/`, creates draft sub-issues with dependencies, and opens a PR. Sub-issues are labeled `draft` until the design PR merges. (**Note:** the label-driven `AGENT_ACTION=design` step is not yet implemented; `agent:design` is a placeholder trigger until issue #71 lands. `agent-design.yml` does exist and runs the un-draft job when a `design/issue-{N}` PR merges, removing the `draft` label from all sub-issues of the parent issue.)
...
- `agent:design` — triggers the designer agent (`AGENT_ACTION=design`) to write a design document on a `design/issue-{N}` branch, open a PR, and create sub-issues labeled `draft` with dependency tracking. Intended for issues the groomer classifies as `plan`. (**Note:** the label-driven designer-agent action (`AGENT_ACTION=design`) is not yet implemented; this label is a placeholder trigger until issue #71 lands. The `agent:design` label is also not yet in `terraform/main.tf`; it will be added in issue #69 — on a fresh repo before that lands, create the label manually.) The `agent-design.yml` workflow is implemented and runs the merge-triggered un-draft job: when a `design/issue-{N}` PR merges, it automatically removes the `draft` label from all sub-issues of the parent issue, unblocking the developer agent for each one.

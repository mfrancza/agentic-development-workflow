...
Optional: `AGENT_MODEL` (default `sonnet`), `AGENT_MAX_TURNS` (default `100`), `ESCALATION_ASSIGNEE` (GitHub username to assign when `resolve-conflicts` applies the `human-required` label; omit to label without assigning), `ADMIN_ASSIGNEES` (JSON array of GitHub usernames to assign when `groom` applies the `human-required` label to an issue, e.g. `["alice","bob"]`; default empty string — label is applied without assigning anyone), `CODE_REVIEWERS` (JSON array of GitHub usernames to request as PR reviewers when `implement` escalates to human review, e.g. `["alice","bob"]`; default empty string — review is not requested from anyone).

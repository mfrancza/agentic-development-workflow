...
2. Applying the `agent:developer` label triggers the developer agent (`AGENT_ACTION=implement`), which creates the `agent/issue-{N}` branch, writes a solution, and opens a PR. The workflow skips with a log line if the issue is labeled `draft` (see the `draft` label description in **Labels**).

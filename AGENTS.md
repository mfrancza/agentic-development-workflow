...
These are recurring false-positive or misread diagnostics that have already caused incidents in this repo. Read this list before acting on any CI or linter finding that seems to flag a core workflow pattern.

- **`github.job_workflow_sha` — actionlint false positive.** actionlint's bundled context schema does not include `job_workflow_sha` on the `github` context object. It therefore reports a spurious "property `job_workflow_sha` is not defined in object type" (or "undefined variable `job_workflow_sha`") diagnostic on any workflow that references `github.job_workflow_sha`. **This is a false positive.** `github.job_workflow_sha` is a GitHub-documented context property populated for reusable-workflow runs to the resolved SHA the caller's `@<ref>` pointed to at dispatch time. The correct expression is `github.job_workflow_sha` — do **not** "fix" it by removing or renaming it. On 2026-09-09 a previous agent chain took this false positive as ground truth, replaced the working expression with the non-existent `job.workflow_sha`, and cascaded the error across four PRs (#522–#525) before a revert (#527) restored the working state. `.github/actionlint.yaml` ignores the diagnostic permanently. See [`docs/design/job-workflow-sha-linter-trap.md`](docs/design/job-workflow-sha-linter-trap.md) for the full postmortem and the four preventive decisions.

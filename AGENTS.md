...
- `agent:review` — applied to a PR to request a review from the code review agent. Triggers `agent-review.yml`, which builds `docker/reviewer/` and runs the reviewer container with the `reviewer-agent` App identity.
  ...
  - Model selection uses the same two-tier waterfall as issue-driven runs: (1) check for a `model:review:*` label — if exactly one matches, use that model; (2) fall back to a generic `model:*` label; (3) fall back to `vars.DEFAULT_MODEL`. The workflow fails loudly if more than one label matches at either tier.

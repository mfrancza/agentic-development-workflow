...
Provider/key mapping: `ANTHROPIC_API_KEY` for Anthropic models (e.g. `model:sonnet`, `model:opus`, `model:haiku`); `OPENAI_API_KEY` for OpenAI models (e.g. `model:o3`); `XAI_API_KEY` for xAI Grok models (e.g. `model:grok-code-fast-1`). The entrypoint infers the provider from the resolved model name and validates that the corresponding key is set.

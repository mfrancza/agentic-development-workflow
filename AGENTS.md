...
- `model:<agent-type>:<name>` (e.g. `model:developer:opus`, `model:groom:haiku`, `model:design:claude-sonnet-4-5`) — per-agent model override for issue-driven runs. When an issue carries both a per-agent label and a generic `model:*` label, the per-agent label wins; the generic label is ignored for that run. Resolution waterfall for issue-driven workflows: (1) check for `model:<agent-type>:*` labels — if exactly one matches, use that model; (2) fall back to generic `model:*` labels (matched by `^model:[^:]+$`, which excludes per-agent labels with a second colon); (3) fall back to `vars.DEFAULT_MODEL`. Fail loudly if more than one label matches at either tier. Agent types are `developer` (used by `agent-implement` and `agent-fix-deployment`), `groom` (used by `agent-groom`), and `design` (used by `agent-design`). PR-based workflows (`agent-review`, `agent-fix-checks`, `agent-respond-review`) use the single-tier `model:*` resolution and ignore per-agent labels.

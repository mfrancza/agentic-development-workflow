...
3. Fetch the issue's current labels (`gh issue view "$GITHUB_ISSUE_NUMBER" --json labels --repo "$GITHUB_REPO"`).
   ...
   - If **no** `model:*` label is present, choose **exactly one** `model:*` label from the criteria — always one of the three tier aliases (`model:sonnet`/`model:opus`/`model:haiku`), never a generic series tag or a pinned snapshot — and apply it **before** processing any other labels.

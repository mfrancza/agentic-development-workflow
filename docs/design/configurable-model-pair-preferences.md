...
```json
{
  "classes": {
    ...
    "sonnet": {
      ...
      "xai":       ["grok-4.20-0309-non-reasoning", "grok-4.5", "grok-4.6"]
    },
    "opus": {
      "anthropic": ["opus"],
...
|-------|------|------------|
| [#469](https://github.com/mfrancza/agentic-development-workflow/issues/469) | Add `.github/scripts/data/model-classes.json` with the initial class table (haiku/sonnet/opus × three vendors) plus a shared TypeScript loader `.github/scripts/src/lib/model-classes.ts` and a Vitest snapshot test that verifies canonical picks. Add a "Cross-vendor class table" section to `docs/model-guidance.md` that mirrors the JSON. | — |

...
Each image is tagged with the exact version (`v1.2.3`), the moving major
(`v1`), and — for the tip of `main` — a `main` tag used by this repo's own
self-hosted workflows during dev.
...
| [#399](https://github.com/mfrancza/agentic-development-workflow/issues/399) | Composite-action portability: change every TypeScript-running action in `.github/actions/*` to run `npm ci`/`npx tsx` with `working-directory: ${{ github.action_path }}/../../scripts`. Update the workflow-activity conventions doc in AGENTS.md to record the new pattern. Add a self-test action-invocation from a sibling composite to confirm the resolution works both from a workspace-local reference and from a hypothetical remote reference (simulated by running the action from a temporary directory). | — |
| [#400](https://github.com/mfrancza/agentic-development-workflow/issues/400) | Container image publishing: add `.github/workflows/release-images.yml` that builds `docker/Dockerfile` and `docker/reviewer/Dockerfile` on `push: tags: 'v*'` and pushes to GHCR at `<tag>` and `v<major>`. Extend `.github/actions/run-agent/action.yml` with an `image` input; when set, `docker pull` + `docker run` the image; when empty, keep today's `docker build "${build-context}"`. Preserve today's self-hosted build-from-source path in this repo's own workflows. | Issue #399 (portability needed for pulls-only path) |

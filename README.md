...
- Apply **`agent:review`** to a PR → the code review agent reviews the changes. (`agent-review.yml` builds `docker/reviewer/` and runs the reviewer container with the `reviewer-agent` App identity.) GitHub suppresses `pull_request_target` events (specifically the `synchronize` type) while a PR has merge conflicts; remove and re-apply `agent:review` after resolving conflicts to restart the re-review loop.

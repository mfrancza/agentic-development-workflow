...
- Apply **`agent:review`** to a PR → the code review agent reviews the changes (and re-reviews when the PR is updated).
...
- Create the labels consumed by the agent workflows (`agent:developer`, `agent:groom`, `agent:review`, `model:sonnet`/`opus`/`haiku`, and the grooming labels `question`/`bug`/`enhancement`/`dependency upgrade`/`do`/`plan`) so they show up in the GitHub label picker on issue creation.

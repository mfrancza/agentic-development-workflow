...
- Apply **`agent:review`** to a PR → the code review agent reviews the changes. (`agent-review.yml` builds `docker/reviewer/` and runs the reviewer container with the `reviewer-agent` App identity.)
  **Note:** GitHub suppresses `pull_request`/`pull_request_review` workflow runs while a PR has merge conflicts; remove and re-add the `agent:review` label after resolving conflicts to restart the review loop.

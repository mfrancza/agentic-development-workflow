...
- Apply **`agent:review`** to a PR → the code review agent reviews the changes. (`agent-review.yml` builds `docker/reviewer/` and runs the reviewer container with the `reviewer-agent` App identity.) Re-apply the label after resolving merge conflicts to restart the review loop, as GitHub suppresses review events during conflicts.
